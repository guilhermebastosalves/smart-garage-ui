...
//     if (loading) {
//         return <div className="text-center mt-5"><div className="spinner-border" role="status"><span className="visually-hidden">Carregando...</span></div></div>;
//     }
...
//     if (!detalhes) {
//         return <div className="alert alert-warning container mt-5">Nenhuma informação encontrada para esta consignação.</div>;
//     }

//     // Variáveis auxiliares para facilitar o acesso aos dados
//     const { automovel, cliente } = detalhes;
//     // Pega o primeiro modelo do array de modelos
//     const modeloDoAutomovel = automovel?.modelos?.[0];
...
//                     <div>
//                         <h1 className="fw-bold mb-0">Detalhes da Consignação</h1>
//                         <p className="text-muted">Informações completas do registro.</p>
//                     </div>
...
//                                 <h5 className="mb-0 fw-bold">Informações da Consignação</h5>
//                             </div>
//                             <div className="card-body">
//                                 <p><strong>ID:</strong> {detalhes.id}</p>
//                                 <p><strong>Data de Início:</strong> {new Date(detalhes.data_inicio).toLocaleDateString('pt-BR', { timeZone: 'UTC' })}</p>
//                                 <p><strong>Data de Término:</strong> {detalhes.data_termino ? new Date(detalhes.data_termino).toLocaleDateString('pt-BR', { timeZone: 'UTC' }) : 'N/A'}</p>
//                                 <p><strong>Valor Acordado:</strong> <span className="text-success fw-bold">{detalhes.valor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></p>
//                                 <p><strong>Status:</strong> {detalhes.ativo ? <span className="badge bg-success">Ativa</span> : <span className="badge bg-secondary">Inativa</span>}</p>
//                             </div>
//                         </div>
//                     </div>
...
//                             </div>
//                             <div className="card-body">
//                                 <p><strong>Marca:</strong> {automovel?.marca?.nome}</p>
//                                 <p><strong>Modelo:</strong> {modeloDoAutomovel?.nome || 'N/A'}</p>
//                                 <p><strong>Ano/Modelo:</strong> {`${automovel?.ano_fabricacao}/${automovel?.ano_modelo}`}</p>
...
//                                     {cliente?.fisica?.cpf && <p><strong>CPF:</strong> {cliente?.fisica?.cpf}</p>}
//                                     {cliente?.juridica?.cnpj && <p><strong>CNPJ:</strong> {cliente?.juridica?.cnpj}</p>}
//                                     {cliente?.juridica?.razao_social && <p><strong>Razão Social:</strong> {cliente?.juridica?.razao_social}</p>}
                    ...
                    <div className="col-lg-6 col-md-12">
                        <div className="card shadow-sm h-100 border-start border-secondary border-4">
                            ...
                            <div className="card-body">
                                ...
                                <p className="mb-0"><strong>Cor:</strong> {automovel?.cor || 'N/A'}</p>
                            </div>
                        </div>
                    </div>
